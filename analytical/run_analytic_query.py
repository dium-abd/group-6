...
def run_query(filepath, warmup_reps, repetitions):
    ...
    print(f"\n== Executando {filepath} {repetitions}x ({warmup_reps}x de aquecimento) ==\n")
    ...
    for i in range(warmup_reps):
        ...
        print(f"Warm-up {i+1}: {exec_time:.3f} ms" if exec_time else "Erro ao extrair tempo!")
        cur.close()
    ...
    print(f"\n🏁 Tempo médio (EXPLAIN): {avg:.3f} ms em {repetitions} execuções.")

    # Encontra o plano mais próximo da média
    closest_run = min(all_runs, key=lambda x: abs(x[0] - avg))
    print(f"\n📌 Plano mais próximo da média ({closest_run[0]:.3f} ms):\n")
    print(closest_run[1])
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Uso: python run_analytic_query.py qX.sql 2 5")
        sys.exit(1)

    filepath = sys.argv[1]
    warmup_reps = int(sys.argv[2])
    reps = int(sys.argv[3])

    run_query(filepath, warmup_reps, reps)
